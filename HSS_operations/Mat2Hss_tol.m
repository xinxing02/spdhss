function [D, U, B] = Mat2Hss_tol(A, htree, tol)
%   Standard HSS approximation 
%   For symmetric matrix A with a given relative error threshold tol for the off-diagonal blocks. 
%   
    ...
    for i = nlevel : -1 : 2          
        ...
        for j = 1 : length(level{i})
            ...
            if (isnan(children(node,1)))                
                ...
                U{node} = randSVDtol(A(ind, xorind), tol(i));           
            %   For Non-Leaf Node
            else 
                c1 = children(node,1);
                c2 = children(node,2);
                ...
                U{node} = randSVDtol(A(ind12, xorind), tol(i));                          
            end            
        end
