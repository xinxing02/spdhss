function [D, U, B] = Mat2Hss_SPD_rank(A, htree, r)
%   SPD HSS Method 1
%
%   SPD HSS approximation of an SPD matrix A with FIXED APPROXIMATION RANK 
...
%               (can be an array defiing different ranks in different levels)
%   
%   Output: 
    ...
    for i = nlevel : -1 : 2  
        for j = 1 : length(level{i})
            ...
            if (isnan(children(node,1)))                
                ...
                [V, e] = eig(D{node});
                e = diag(e);
                A(idx, xorind) = V' * A(idx, xorind);  %onsite modification
                score = sum(A(idx, xorind).^2, 2);
                [~,I] = sort(score, 'descend');
                I1 = I(1:min(r(i), size(V,1)));
                Ic = I(min(r(i),size(V,1))+1:end);
                
                %   Generator U and diagonal block
                U{node} = V(:, I1);
                tmpE{node} = e(I1); 
                
                %   Effective rows after the onsite modification
                effect_idx_per_node{node} = idx(1)-1+I1;               
                effect_rowidx(idx(1)-1+Ic) = false;
        ...
        for j = 1 : length(level{i})
            ...
            xorind(idx) = false;
            if (isnan(children(node,1))) 
                A(xorind, effect_idx_per_node{node}) = A(xorind, idx) * U{node};
            else
                c1 = children(node,1);
                c2 = children(node,2);
